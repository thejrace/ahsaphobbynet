<?php
	...
	$TEMPLATE = "deleted_products";
	$AJAX_REQ = "ajax/ajax_deleted_products.php";
	...
	if( Input::get("katid") != "" ) {
		...
		if( $Category->exists() ) {
			...
			$Fetch->get_data( "search", $Product_Table->get_settings(), array( " WHERE " . $Fetch->create_syntax( "category", "||", $cat_count ) , $C_Tree->get_list() ) );
		} else {
